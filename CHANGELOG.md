...
- Built-in Arabic reading font: Noto Naskh Arabic (regular and bold, four sizes) now ships in the firmware itself, so Arabic books and menus render fully — shaping, harakat, kashida — with no SD-card fonts installed. The automatic language-font switch falls back to it when the card has no Arabic font.
- RTL books turn pages right-to-left: the prev/next front buttons swap sides, the bottom hint labels follow, and on touch devices the tap zones and swipe directions mirror. The book's OPF language decides; untagged books and plain text follow the UI language, and Latin books keep left-to-right turns even under an Arabic UI.
- Switching the UI language across the Arabic/Latin boundary now switches the reader font automatically: each script class remembers your last font choice, a first switch to Arabic auto-selects an installed Arabic-capable family (Noto Naskh Arabic, then Amiri, then any family that covers Arabic), and switching back restores your previous Latin font.
